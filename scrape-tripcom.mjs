...
function regexCapture(text, patterns) {
...
async function domValueForLabels(page, labels) {
  return await page.evaluate((labels) => {
    ...
    const elements = Array.from(
      document.querySelectorAll(
        "div,span,p,dt,dd,li,th,td,strong,b,label"
      )
    );

    for (const element of elements) {
      const labelText = clean(element.textContent);

      if (!wanted.includes(labelText.toLowerCase())) continue;
      ...
      if (parent) {
        ...
        if (
          parentText &&
          parentText.toLowerCase() !== labelText.toLowerCase()
        ) {
          const escaped = labelText.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&"
          );
...
async function extractOperationalDetails(page, cached = {}) {
  const bodyText = await page.locator("body").innerText();
  const fullText = clean(bodyText);
  ...
  if (!checkInCounter) {
    checkInCounter = valueAfterLabel(lines, [
      "Check-in Counter",
      "Check-in counter",
      "Check in Counter",
      "Check in counter"
  ...
  if (!checkInCounter) {
    checkInCounter = regexCapture(fullText, [
      /Check-?in\s+Counter\s*[:：]?\s*([A-Z0-9][A-Z0-9 ,/&-]{0,40})/i,
      /Check\s+in\s+Counter\s*[:：]?\s*([A-Z0-9][A-Z0-9 ,/&-]{0,40})/i
    ]);
  }
  ...
  if (!checkInTimeRaw) {
    checkInTimeRaw = valueAfterLabel(lines, [
      "Check-in Time",
      "Check-in time",
      "Check in Time",
      "Check in time"
  ...
  if (!checkInTimeRaw) {
    checkInTimeRaw = regexCapture(fullText, [
      /Check-?in\s+(?:Time\s*)?[:：]?\s*((?:\d{4}-\d{2}-\d{2}\s+)?\d{1,2}:\d{2}\s*(?:-|–|—|to)\s*(?:\d{4}-\d{2}-\d{2}\s+)?\d{1,2}:\d{2})/i,
      /Check-?in\s+closes?\s+(?:at\s+)?(?:\d{4}-\d{2}-\d{2}\s+)?(\d{1,2}:\d{2})/i,
      /Check\s+in\s+closes?\s+(?:at\s+)?(?:\d{4}-\d{2}-\d{2}\s+)?(\d{1,2}:\d{2})/i
    ]);
  }
  ...
  if (!boardingGate) {
    boardingGate = valueAfterLabel(lines, [
      "Boarding Gate",
      "Boarding gate",
      "Departure Gate",
      "Departure gate"
  ...
  if (!boardingGate) {
    boardingGate = regexCapture(fullText, [
      /Boarding\s+Gate\s*[:：]?\s*([A-Z0-9][A-Z0-9-]{0,12})/i,
      /Departure\s+Gate\s*[:：]?\s*([A-Z0-9][A-Z0-9-]{0,12})/i
    ]);
  }
  ...
  if (!arrivalGate) {
    arrivalGate = valueAfterLabel(lines, [
      "Arrival Gate",
      "Arrival gate"
    ]);
  }
  ...
  if (!arrivalGate) {
    arrivalGate = regexCapture(fullText, [
      /Arrival\s+Gate\s*[:：]?\s*([A-Z0-9][A-Z0-9-]{0,12})/i
    ]);
  }
  ...
  if (!baggage) {
    baggage = regexCapture(fullText, [
      /Baggage\s+Belt\s*[:：]?\s*([A-Z0-9][A-Z0-9 -]{0,20})/i,
      /Baggage\s+Claim\s*[:：]?\s*([A-Z0-9][A-Z0-9 -]{0,20})/i,
      /Carousel\s*[:：]?\s*([A-Z0-9][A-Z0-9 -]{0,20})/i,
      /Baggage\s*[:：]?\s*(Belt\s*[A-Z0-9]+|\d{1,3})/i
    ]);
  }

  return {
    checkInCounter:
      clean(checkInCounter) ||
      cached?.checkInCounter ||
      "TBA",
...
async function scrapeFlightDetails(context, row, cached) {
  ...
  try {
    ...
    return await extractOperationalDetails(page, cached);
